...
"""
Usage:
        bvccli  show-nodes       [options]
        bvccli  show-hosts       [options]
        bvccli  show-flows       [options]
        bvccli  show-modules     [options]
        bvccli  show-mounts      [options]
        bvccli  http-get         [options] <uri>
        bvccli  show-api         [options]
        bvccli  get-capabilities [options] <node>
        bvccli  get-config       [options] <node> (--ops | --config)
        bvccli  get-schema       [options] <node> <module>
        bvccli  get-allschemas   [options] <node>
        bvccli  post-netconf     [options] <resource> [--node <node>] (--ops | --config) [--payload <json>]
        bvccli  mount-device     [options] <name> <address> <user> <password> [--port <port>] [--mlx]
        bvccli  unmount-device   [options] <name>
...
class BvcCLI(object):
    ...
    def show_mounts(self, args):
        ...
        if status:
            utils.print_table("show-mounts", retval, 'name')
        else:
            print("Houston we have a problem, {}").format(retval)
