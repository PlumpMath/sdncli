...
class Controller(object):
    ...
    def get_bvc_nodes(self, ds, debug=False):
    ...
    def netconf_get(self, r, ds, api):
        ...
        try:
            retval = self.session.get(resource, auth=self.auth, params=None, headers=self.headers,  timeout=120)
        except requests.exceptions.ConnectionError:
            return (("Error connecting to BVC {}").format(self.server), False)
        if str(retval.status_code)[:1] == "2":
            try:
    ...
    def get_bvc_hosts(self, debug=False):
        '''
    ...
    def get_modules(self, debug=False):
        moduledb = {}
    ...
    def get_capabilities(self, node, debug=False):
        moduledb = {}
        ds = "operational"
    ...
    def get_mounts(self, debug=False):
        mountdb = {}
        (retval, status) = self._get_mount_status(debug)
        mntstats = retval
        if status:
    ...
    def _get_mount_status(self, debug=False):
        stats = {}
    ...
    def mount_netconf_device(self, name, node, user, pw, port, mlx=None):
        if not port:
            port = 830
        if mlx:
